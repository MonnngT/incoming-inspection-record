...
SKIP_LOT_SUPPLIERS = {"SP", "AQ", "金*"}

# 兼容Google Sheets中的旧供应商名称，界面及后续记录统一使用SP。
SUPPLIER_ALIASES = {"速锐达(SP)": "SP", "速锐达": "SP"}


def normalize_supplier(value):
    supplier = str(value).strip()
...
def load_history(ws):
    ...
    # 历史记录中的旧名称在内存中统一为SP，确保原有记录继续参与累计和跳批判定。
    df["供应商"] = df["供应商"].map(normalize_supplier)
    return df
