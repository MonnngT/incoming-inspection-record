...
- **执行动作**：根据跳批规则自动判定
  ...
  - 不参加跳批的供应商每批固定显示 `正常检验`
- **历史记录自动校正**：程序启动后自动补全原有记录的累计批次数和执行动作
- **检验用时**：分"开始时间 / 结束时间"两个输入，自动算出用时（分钟）
- **结果**：下拉 OK / NG
- **检具检验**：盘类检查内径检具、扇叶检查轮毂外径检具、Boss类检查法兰检具
...
4. 跳批仅免做 **尺寸数值** 检验；外观、包装数量及关键检具每批必检
5. 检具结果为 `NG` 时，该批最终结果自动判定为 `NG`，并退出跳批状态
